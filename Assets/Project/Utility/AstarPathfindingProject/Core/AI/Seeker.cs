...
namespace Pathfinding 
{
	...
	public class Seeker : VersionedMonoBehaviour 
	{
		...
		///     This path can be sent to <see cref="StartPath(Path,OnPathDelegate,int)" /> with no change, but if no change is
		///     required <see cref="StartPath(Vector3,Vector3,OnPathDelegate)" /> does just that.
		///     <code>
		...
		///     Since this method does not take a callback parameter, you should set the <see cref="pathCallback" /> field before
		///     calling this method.
		/// </summary>
		...
		///     Callback will not be called if the path is canceled (e.g when a new path is requested before the previous one has
		///     completed)
		/// </summary>
		/// <param name="start">The start point of the path</param>
		/// <param name="end">The end point of the path</param>
		...
		/// <param name="end">The end point of the path</param>
		/// <param name="callback">The function to call when the path has been calculated</param>
		/// <param name="graphMask">
		///     Mask used to specify which graphs should be searched for close nodes. See
		///     #Pathfinding.NNConstraint.graphMask. This will override #graphMask for this path request.
		/// </param>
		...
		///     Version: Since 4.1.x this method will no longer overwrite the graphMask on the path unless it is explicitly passed
		///     as a parameter (see other overloads of this method).
		/// </summary>
		...
		/// <param name="graphMask">
		///     Mask used to specify which graphs should be searched for close nodes. See
		///     #Pathfinding.GraphMask. This will override #graphMask for this path request.
		/// </param>
		...
		private void StartPathInternal(Path p, OnPathDelegate callback)
		{
			...
			if (mtp != null)
			{
				...
				for (var i = 0; i < callbacks.Length; i++) callbacks[i] = onPartialPathDelegate;
		...
		///     A Multi Target Path will search for all the end points in one search and will return all paths if pathsForAll is
		///     true, or only the shortest one if pathsForAll is false.
		///     callback and <see cref="pathCallback" /> will be called when the path has completed. Callback will not be called if
		///     the path is canceled (e.g when a new path is requested before the previous one has completed)
		///     See: Pathfinding.MultiTargetPath
		///     See: MultiTargetPathExample.cs (view in online documentation for working links) "Example of how to use
		///     multi-target-paths"
		/// </summary>
		/// <param name="start">The start point of the path</param>
		/// <param name="endPoints">The end points of the path</param>
		/// <param name="pathsForAll">
		///     Indicates whether or not a path to all end points should be searched for or only to the
		///     closest one
		/// </param>
		/// <param name="callback">The function to call when the path has been calculated</param>
		/// <param name="graphMask">
		///     Mask used to specify which graphs should be searched for close nodes. See
		///     Pathfinding.NNConstraint.graphMask.
		/// </param>
		public MultiTargetPath StartMultiTargetPath(Vector3 start, Vector3[] endPoints, bool pathsForAll,
			OnPathDelegate callback = null, int graphMask = -1)
		{
			var p = MultiTargetPath.Construct(start, endPoints, null);
		...
		///     A Multi Target Path will search from all start points to the target point in one search and will return all paths
		///     if pathsForAll is true, or only the shortest one if pathsForAll is false.
		///     callback and <see cref="pathCallback" /> will be called when the path has completed. Callback will not be called if
		///     the path is canceled (e.g when a new path is requested before the previous one has completed)
		///     See: Pathfinding.MultiTargetPath
		///     See: MultiTargetPathExample.cs (view in online documentation for working links) "Example of how to use
		...
		/// <param name="end">The end point of the path</param>
		/// <param name="pathsForAll">
		///     Indicates whether or not a path from all start points should be searched for or only to the
		///     closest one
		/// </param>
		/// <param name="callback">The function to call when the path has been calculated</param>
		/// <param name="graphMask">
		...
		///     Pathfinding.NNConstraint.graphMask.
		/// </param>
		public MultiTargetPath StartMultiTargetPath(Vector3[] startPoints, Vector3 end, bool pathsForAll,
			OnPathDelegate callback = null, int graphMask = -1)
		{
			var p = MultiTargetPath.Construct(startPoints, end, null);
		...
		public MultiTargetPath StartMultiTargetPath(MultiTargetPath p, OnPathDelegate callback = null,
			int graphMask = -1)
		{
			StartPath(p, callback, graphMask);
			return p;
