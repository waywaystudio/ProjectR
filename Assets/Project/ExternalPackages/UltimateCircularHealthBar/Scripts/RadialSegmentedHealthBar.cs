...
namespace RengeGames.HealthBars
{
    ...
    public class UniquePBMaterialValidator
    {
        ...
        {
            if (!materialName.Contains(RadialSegmentedHealthBar.MaterialNamePrefix)) return false;
            Refresh();
            bool isStored = instances.Contains(instance);
            if (uniqueMaterials.Contains(materialName) && !isStored) return false;
    ...
    [ExecuteAlways] [DisallowMultipleComponent] [AddComponentMenu("Health Bars/Circular Segmented Health Bar")]
    public class RadialSegmentedHealthBar : MonoBehaviour, ISegmentedHealthBar
    {
        ...
        public string ParentName
        {
            ...
            set
            {
                if (Application.isPlaying) StatusBarsManager.RemoveHealthBar(this, false);
                parentName = value;
                if (Application.isPlaying) StatusBarsManager.AddHealthBar(this);
            }
        }
        ...
        [SerializeField] public float updatesPerSecond = 30;
        ...
        private string BaseMaterialName
        {
            get
            {
                if (!forceBuiltInShader && GraphicsSettings.renderPipelineAsset && Int32.Parse(Application.unityVersion.Split('.')[0]) > 2019) return "RadialSegmentedHealthBarMaterial";
        ...
        private void OnValidate()
        {
            ...
            ValidateComponents();
            if (materialAssigned) ApplyToShader(false);
        }
        ...
        private void OnEnable() { StartCoroutine("UpdateShader"); }
        ...
        {
            if (materialAssigned && !setInShader) return Convert.ToBoolean(ActiveMaterial.GetFloat(id));
        ...
        {
            if (materialAssigned && !setInShader) return ActiveMaterial.GetFloat(id);
            if (materialAssigned && setInShader)
            {
        ...
        {
            if (materialAssigned && !setInShader) return ActiveMaterial.GetColor(id);
            if (materialAssigned && setInShader)
            {
        ...
        {
            if (materialAssigned && !setInShader) return ActiveMaterial.GetVector(id);
            if (materialAssigned && setInShader)
            {
        ...
        {
            if (materialAssigned && !setInShader) return (Texture2D)ActiveMaterial.GetTexture(id);
            if (materialAssigned && setInShader)
            {
        ...
        {
            if (materialAssigned && !setInShader) return value;
            if (materialAssigned && setInShader)
            {
        ...
        {
            if (materialAssigned && !setInShader) return ActiveMaterial.IsKeywordEnabled(id);
            if (materialAssigned && setInShader && value)
                ActiveMaterial.EnableKeyword(id);
            else if (materialAssigned && setInShader && !value) ActiveMaterial.DisableKeyword(id);
            return false;
        }
...
#if UNITY_EDITOR
                //the scene needs to be saved
                if (!Application.isPlaying && !EditorUtility.IsPersistent(this)) UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(UnityEngine.SceneManagement.SceneManager.GetActiveScene());
#endif
            }
        }
        ...
        public void SetSegmentCount(float value) { SegmentCount.Value = Mathf.Max(0, value); }

        public void SetRemovedSegments(float value) { RemoveSegments.Value = Mathf.Clamp(value, 0, SegmentCount.Value); }
        ...
        public string GetName() { return hbName; }
    }
}