...
namespace RengeGames.HealthBars
{
    public class StatusBarsManager
    {
        ...
        private StatusBarsManager() { _healthBars = new Dictionary<string, Dictionary<string, List<ISegmentedHealthBar>>>(); }

        public bool IsEnabled() { return InternalInstance.enabled; }
        public void Disable() { InternalInstance.enabled = false; }
        public void Enable() { InternalInstance.enabled  = true; }
        public void Clear() { InternalInstance._healthBars.Clear(); }
        ...
        public static void RemoveHealthBar(ISegmentedHealthBar healthBar, bool removeAll)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(healthBar.GetParentName(), out var parent))
            {
                ...
                else if (parent.TryGetValue(healthBar.GetName(), out var hbs))
                {
                    ...
                    if (hbs.Count == 0) parent.Remove(healthBar.GetName());
                }
                else
                {
                    removed = false;
                ...
                if (removed && parent.Count == 0) InternalInstance._healthBars.Remove(healthBar.GetParentName());
            }
        }
        ...
        public static void RemoveHealthBar(ISegmentedHealthBar healthBar, string parentName, string name, bool removeAll)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                ...
                {
                    removed = hbs.Remove(healthBar);
                    if (hbs.Count == 0) parent.Remove(name);
                }
                else
                {
                    removed = false;
                ...
                if (removed && parent.Count == 0) InternalInstance._healthBars.Remove(parentName);
            }
        }
        ...
        public static void SetSegmentCount(string parentName, string healthBarName, float value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) hbs.ForEach((hb) => hb.SetSegmentCount(value));
            }
        }
        ...
        public static void SetRemovedSegments(string parentName, string healthBarName, float value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) hbs.ForEach(hb => hb.SetRemovedSegments(value));
            }
        }
        ...
        public static void SetPercent(string parentName, string healthBarName, float value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) hbs.ForEach(hb => hb.SetPercent(value));
            }
        }
        ...
        public static void AddRemoveSegments(string parentName, string healthBarName, float value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) hbs.ForEach(hb => hb.AddRemoveSegments(value));
            }
        }
        ...
        public static void AddRemovePercent(string parentName, string healthBarName, float value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) hbs.ForEach(hb => hb.AddRemovePercent(value));
            }
        }
        ...
        public static bool GetShaderPropertyValue<T>(string parentName, string healthBarName, string propertyName, out T value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) return hbs[0].GetShaderPropertyValue(propertyName, out value);
            }
        ...
        public bool GetShaderKeywordValue(string parentName, string healthBarName, string propertyName, out bool value)
        {
            ...
            if (InternalInstance._healthBars.TryGetValue(parentName, out var parent))
            {
                if (parent.TryGetValue(healthBarName, out var hbs)) return hbs[0].GetShaderKeywordValue(propertyName, out value);
            }
