...
namespace BehaviorDesigner.Runtime
{
    public class JSONDeserialization : UnityEngine.Object
    {
        ...
        public static void Load(TaskSerializationData taskData, BehaviorSource behaviorSource, bool loadTasks)
        {
            ...
            // Local behavior trees do not need to consume external behavior tree tasks at edit time.
            if (!loadTasks) {
                return;
            }
            ...
            if (taskIDs != null && taskIDs.Count > 0) {
                foreach (TaskField taskField in taskIDs.Keys) {
                    ...
                    } else if (IDtoTask.TryGetValue(idList[0], out var idTask)) { // single task
                        if (idTask.GetType().Equals(taskField.fieldInfo.FieldType) || idTask.GetType().IsSubclassOf(taskField.fieldInfo.FieldType)) {
                            taskField.fieldInfo.SetValue(taskField.task, idTask);
                        }
                    }
                }
                taskIDs = null;
                ...
                if (dict.TryGetValue(fieldKey, out dictObject)) {
                    if (typeof(IList).IsAssignableFrom(fields[i].FieldType)) {
                        ...
                        if (objectValues != null) {
                            ...
                            bool isIDList = (type.Equals(typeof(Task)) || type.IsSubclassOf(typeof(Task))) && !TaskUtility.HasAttribute(fields[i], typeof(InspectTaskAttribute));
                            if (isIDList) {
                                if (taskIDs != null) {
                                ...
                                if (fields[i].FieldType.IsArray) {
                                    ...
                                    for (int j = 0; j < objectValues.Count; ++j) {
                                        ...
                                        Type elementType;
                                        object value;
                                        if (objectValues[j] is Dictionary<string, object>) {
                                            var elementDict = (Dictionary<string, object>)objectValues[j];
                                            elementType = TaskUtility.GetTypeWithinAssembly((string)elementDict["Type"]);
                                            if (!elementDict.TryGetValue("Value", out value)) {
                                                value = objectValues[j];
                                            }
                                        } else {
                                            elementType = type;
                                            value = objectValues[j];
                                        }
                                        var element = ValueToObject(task, elementType, value, variableSource, unityObjects);
                                    ...
                                    for (int j = 0; j < objectValues.Count; ++j) {
                                        ...
                                        object objVal = objectValues[j];
                                        if (objVal is Dictionary<string, object>) {
                                            var elementDict = (Dictionary<string, object>)objVal;
                                            object elementObj;
                                            if (elementDict.TryGetValue("Type", out elementObj)) {
                                                elementType = TaskUtility.GetTypeWithinAssembly((string)elementObj);
                                                if (!elementDict.TryGetValue("Value", out objVal)) {
                                                    objVal = objectValues[j];
                                                }
                                            }
                                        }

                                        var value = ValueToObject(task, elementType, objVal, variableSource, unityObjects);
