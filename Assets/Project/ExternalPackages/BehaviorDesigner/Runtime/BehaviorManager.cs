    ...
    public class BehaviorManager : MonoBehaviour
    {
            ...
            for (int i = 0; i < fields.Length; ++i) {
                ...
                if (TaskUtility.HasAttribute(fields[i], typeof(InspectTaskAttribute))) {
                    if (typeof(IList).IsAssignableFrom(fields[i].FieldType)) {
                        Type fieldType;
                        if (typeof(Task).IsAssignableFrom((fieldType = fields[i].FieldType.GetElementType()))) {
                            var list = value as IList<Task>;
                            if (list != null) {
                                for (int j = 0; j < list.Count; ++j) {
                                    OverrideFields(behaviorTree, data, list[j]);
                                }
                            }
                        }
                    } else if (typeof(Task).IsAssignableFrom(fields[i].FieldType)) {
                        OverrideFields(behaviorTree, data, value);
                    }
                }
            }
        ...
        private void FixedTick(BehaviorTree behaviorTree)
        {
            for (int j = behaviorTree.activeStack.Count - 1; j > -1; --j) {
                var taskIndex = behaviorTree.activeStack[j].Peek();
                behaviorTree.taskList[taskIndex].OnFixedUpdate();
            }
        }
        ...
        private TaskStatus RunParentTask(BehaviorTree behaviorTree, int taskIndex, ref int stackIndex, TaskStatus status)
        {
            ...
            if (!parentTask.CanRunParallelChildren() || parentTask.OverrideStatus(TaskStatus.Running) != TaskStatus.Running) {
                ...
                while (parentTask.CanExecute() && (childStatus != TaskStatus.Running || parentTask.CanRunParallelChildren()) && IsBehaviorEnabled(activeBehavior)) {
                    ...
                    if (childIndex == -1) {
                        status = childStatus = TaskStatus.Running;
                        continue;
                    }
