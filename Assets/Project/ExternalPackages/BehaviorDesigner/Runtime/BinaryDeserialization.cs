...
public static class BinaryDeserialization
{
    ...
    {
        Load(behaviorSource.TaskData, behaviorSource, true);
    }
    ...
    private static object LoadField(FieldSerializationData fieldSerializationData, Dictionary<int, int> fieldIndexMap, Type fieldType, string fieldName, int hashPrefix, IVariableSource variableSource, object obj = null, FieldInfo fieldInfo = null)
    {
        ...
        } else if (fieldType.Equals(typeof(UInt64)) || fieldType.Equals(typeof(ulong))) {
            value = BytesToULong(fieldSerializationData.byteDataArray, fieldSerializationData.dataPosition[fieldIndex]);
        } else if (fieldType.Equals(typeof(ushort))) {
            value = BytesToUShort(fieldSerializationData.byteDataArray, fieldSerializationData.dataPosition[fieldIndex]);
