...
public static class DictionaryExtension
{
    ...
    public static bool TryGetValue<TKey, TValue>(this Dictionary<TKey, TValue> table, TKey key, out TValue value,
                                                 bool showFalseDebug)
    {
        var result = table.TryGetValue(key, out value);

        if (showFalseDebug && !result)
        {
            Debug.LogError($"Not Exist {key.ToString()} in {table}. return false");
        }
