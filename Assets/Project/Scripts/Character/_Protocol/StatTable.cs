...
namespace Character
{
    ...
    public class StatTable : Dictionary<StatCode, FloatTable>
    {
        ...
        /// <summary>
        /// UnionWith Function is like TargetTable.Add(CallerTable). 
        /// </summary>
        public void UnionWith(StatTable target)
        {
            target.ForEach(x => TryAdd(x.Key, x.Value));
        }
        

        private float Get(StatCode statCode)
        {
    ...
    public class FloatTable : Dictionary<int, float>
    {
        public float Result
        {
            ...
            {
                var result = 0f;
                foreach (var item in this) result += this[item.Key];
                return result;
            }
        }
